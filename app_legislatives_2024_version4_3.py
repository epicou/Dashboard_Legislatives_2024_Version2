...
# Liste élargie des partis
partis = ['% RN', '% LFI', '% LR', '% ENS', '% DVG', '% REC', '% ECO',
          '% DVC', '% DVD', '% UDI', '% EXG', '% DIV', '% PS']

st.title("Carte interactive - Législatives 2024 Paris (Version 4.3)")
...
# On sélectionne uniquement les colonnes qui existent ET qui sont sélectionnées
colonnes_existantes = [p for p in partis if p in votes_plot.columns and p.replace('% ', '') in parti_selection]

# Sécurisation : si aucune colonne trouvée -> taille_points = 10
