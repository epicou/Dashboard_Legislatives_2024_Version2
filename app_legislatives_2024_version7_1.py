...
st.title("Carte interactive - Législatives 2024 Paris (Version 7.1 — Folium améliorée)")
...
votes_plot["taille_points"] = votes_plot["% dominant"].fillna(0) * 300  # échelle augmentée
votes_plot.loc[votes_plot["taille_points"] < 15, "taille_points"] = 15
...
for idx, row in votes_plot.iterrows():
    CircleMarker(
        ...
        radius=row["taille_points"] / 10,  # augmenté
        color=couleurs_partis.get(row["Parti dominant"], "beige"),
        fill=True,
        fill_opacity=0.8,
        popup=folium.Popup(row["popup"], max_width=400)
    ).add_to(m)
