...
import json
import folium
from folium import Choropleth, CircleMarker, Tooltip
from streamlit_folium import st_folium

st.title("Carte interactive - Législatives 2024 Paris — VERSION 7.3 — Tooltips améliorés")
...
votes_plot["taille_points"] = votes_plot["% dominant"].fillna(0) * 1500
votes_plot.loc[votes_plot["taille_points"] < 30, "taille_points"] = 30

# --- Tooltip (infobulle robuste) ---
def top3_partis(row):
    scores = {p.replace("% ", ""): row[p] for p in partis if p in row.index}
    top3 = sorted(scores.items(), key=lambda x: x[1], reverse=True)[:3]
    txt = ""
    for i, (parti, score) in enumerate(top3):
        txt += f"{i+1}er : {parti} - {round(score*100,1)}% | "
    return txt.rstrip(" | ")

votes_plot["tooltip"] = votes_plot.apply(
    lambda row: f"Quartier : {row['Quartier'].capitalize()} | ID_BVOTE : {row['ID_BVOTE']} | {top3_partis(row)}",
    axis=1
)
...
# --- Points BV avec Tooltip ---
for idx, row in votes_plot.iterrows():
    CircleMarker(
        location=[row["LAT"], row["LON"]],
        radius=row["taille_points"] / 10,
