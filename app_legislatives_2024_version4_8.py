...
# Harmoniser les noms de quartiers en minuscules
votes_df["Quartier"] = votes_df["Quartier"].str.lower()
iris_df["Quartier"] = iris_df["Quartier"].str.lower()

partis = ['% RN', '% LFI', '% LR', '% ENS', '% DVG', '% REC', '% ECO',
          '% DVC', '% DVD', '% UDI', '% EXG', '% DIV', '% PS']

st.title("Carte interactive - Législatives 2024 Paris (Version 4.8)")
...
if len(votes_plot) == 0:
    ...
    # Infobulles propres : PAS de LAT, LON, taille_points. On affiche Quartier, ID_BVOTE et % des partis.
    hover_data = {"Quartier": True, "ID_BVOTE": True}
    for p in partis:
        if p in votes_plot.columns:
            hover_data[p] = votes_plot[p].round(1)

    # Construction du GeoJSON sécurisé
    ...
    # Définir les min et max robustes
