...
st.title("Carte interactive - Législatives 2024 Paris (Version 6.0 — Plotly GO)")
...
# --- Création de la figure Plotly GO ---
fig = go.Figure()

# Choroplèthe IRIS
fig.add_trace(go.Choroplethmapbox(
    geojson=geojson_dict,
    locations=[f["properties"]["IRIS"] for f in geojson_dict["features"]],
    z=[f["properties"]["Valeur"] for f in geojson_dict["features"]],
    colorscale="YlOrRd",
    colorbar_title=indicateur_selection,
    marker_opacity=0.5,
    marker_line_width=0,
...
fig.add_trace(go.Scattermapbox(
    ...
    marker=go.scattermapbox.Marker(
        size=votes_plot["taille_points"],
        color=[couleurs_partis.get(p, "beige") for p in votes_plot["Parti dominant"]],
        opacity=0.8
    ),
    text=votes_plot["hovertext"],
    hoverinfo='text',
